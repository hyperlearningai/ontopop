...
public class OntologyIngestorFunctionAzureHandler extends FunctionInvoker<OntologyIngestorFunctionModel, Boolean> {
	
	/**
	 * Azure handler for the Ontology Ingestor cloud function.
	 ...
	 * OntologyIngestorFunction component which is a Spring Bean with the 
	 * same class name but starting with a lowercase character.
	 * @param request
	 * @param context
	 * @return
	 */
	...
	public HttpResponseMessage ingest(
		...
		OntologyIngestorFunctionModel ontologyIngestorFunctionModel = 
				new OntologyIngestorFunctionModel(
						headers, payload);
		
		// Log the HTTP request headers for debugging purposes
		context.getLogger().info(
				"Ontology Ingestor Function triggered: "
		...
		context.getLogger().info(
				"Ontology ingestion webhook payload: " + payload);
